...
```javascript
// Search for similar errors and past debugging sessions
search_memory(query="[error message keywords] [stack trace type]", limit=10)
search_memory(query="debugging [technology] similar issues", limit=5)
```
...
### Memory Templates for Debuggers

**✅ GOOD Error Memory:**
```javascript
store_memory({
  type: "error",
  content: "React useState not updating in useEffect - stale closure issue",
  error_message: "State shows old value in setTimeout callback",
  solution: "Use functional setState: setState(prev => prev + 1)",
  prevention: "Always use functional updates in async contexts",
  tags: ["react", "hooks", "useState", "closure", "async"],
  project: "project-name"
})
```
...
store_memory({
  type: "error",
  content: "Fixed bug",  // Too vague!
  tags: ["bug"]  // Not searchable!
})
```
